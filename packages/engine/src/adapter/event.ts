  ...
  emitEvent(evtObj, eventType) {
    ...
    switch (eventType) {
      case HammerEvent.touchstart:
        e = getEventTarget(transformEvtObj, 'tap', target);
        this.emit('tap', e);
        break;
      case HammerEvent.pan:
        e = getEventTarget(transformEvtObj, 'panmove', target);
        this.emit('panmove', e);
        break;
      default:
    ...
    if (eventType === HammerEvent.pan) {
      ...
      if (this.isDraging) {
        e = getEventTarget(transformEvtObj, DragEventType.drag, this.dragShape);
        this.emit(DragEventType.drag, e);
      } else if ((isDraggable && distance > DRAG_OFFSET) || timeWindow > DRAG_TIME) {
        this.isDraging = true;
        e = getEventTarget(transformEvtObj, DragEventType.dragstart, this.dragShape);
        this.emit(DragEventType.dragstart, e);
      }
      this.emit('*', e);
    }

    if (eventType === HammerEvent.panend && this.isDraging) {
      e = getEventTarget(transformEvtObj, DragEventType.dragend, this.dragShape);
      this.emit(DragEventType.dragend, e);
      this.emit('*', e);
      this.panStartTime = -1;
      this.startX = 0;
      this.startY = 0;
